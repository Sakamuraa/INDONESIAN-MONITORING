      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <div className="rounded-xl border border-[var(--gh-border)] bg-[var(--gh-surface-raised)] p-4">
          <p className="leading-relaxed text-[var(--gh-text-muted)]">
            {t('footer.integrity', { bmkg: 'BMKG', bnpb: 'BNPB/InaRISK', pvmbg: 'PVMBG/MAGMA Indonesia' })}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-5">
          ...
          <div className="md:col-span-1">
            ...
            <ul className="space-y-2 font-mono">
              {[
                { num: '112', label: t('footer.call_112') },
                { num: '115', label: t('footer.basarnas') },
                { num: '117', label: t('footer.bnpb') },
        ...
        <div className="flex flex-col gap-1 border-t border-[var(--gh-border)] pt-6 text-center text-[11px] text-[var(--gh-text-muted)] sm:flex-row sm:justify-between sm:text-left">
          <span>{t('footer.copy', { y: new Date().getFullYear() })}</span>
          <span className="text-[10px] tracking-wide text-[var(--gh-text-subtle)]">{t('footer.map_note')}</span>
        </div>
      </div>
    </footer>
  );
